...
const Dispatch = () => {
  ...
  const [selectedMohallaPersonCount, setSelectedMohallaPersonCount] =
    useState(0);
  ...
  useEffect(() => {
    if (viewDispatchedData) {
      const data =
        viewDispatchedData.length !== 0 &&
        viewDispatchedData.filter((item) => item.mk_id === mohallaUserId);
      if (data[0]) {
        console.log(data);
      ...
      <ConfigProvider
        ...
        <div
          ...
          {localStorage.getItem("type") === "mk superadmin" ? (
            <Sidebar k="8" userType="superadmin" />
          ) : (
