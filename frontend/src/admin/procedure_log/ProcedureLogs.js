...
import { Button, ConfigProvider, DatePicker, Input, Rate, Tabs } from "antd";
...
const ProcedureLogs = () => {
  ...
  const [msciCharges, setMsciCharges] = useState({
    dailyMaintenanceCharges: 0,
    gasCharges: 0,
    laborCharges: 0,
    transportCharges: 0,
  });

  const [msciChargesTotal, setMsciChargesTotal] = useState(0);

  const onHandleChanges = (e) => {
    const { name, value } = e.target;
    setMsciCharges((prev) => ({ ...prev, [name]: value }));
  };

  const onAddValues = () => {
    fetch("/api/operation_pipeline/addMicsCharges", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        menu_id: menuFoodId,
        dailyMaintenanceCharges: msciCharges?.dailyMaintenanceCharges,
        gasCharges: msciCharges?.gasCharges,
        laborCharges: msciCharges?.laborCharges,
        transportCharges: msciCharges?.transportCharges,
      }),
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    const getMicsCharges = async () => {
      try {
        const res = await fetch("/api/operation_pipeline/getMicsCharges", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        setMsciCharges((prev) => ({
          dailyMaintenanceCharges: data?.dailyMaintenanceCharges,
          gasCharges: data?.gasCharges,
          laborCharges: data?.laborCharges,
          transportCharges: data?.transportCharges,
        }));

        setMsciChargesTotal(data?.micsCharges ? data.micsCharges : 0);
      } catch (error) {
        console.log(error);
      }
    };
    ...
    <div style={{ margin: 0, padding: 0 }}>
      <div
        ...
        {localStorage.getItem("type") === "mk superadmin" ? (
          <SideNav k="2" userType="superadmin" />
        ) : (
                            ...
                            <div
                              ...
                              {dispatchData.length !== 0 &&
                                dispatchData
                                  .filter(
                                    (filterDispatch) =>
                                      filterDispatch?.mk_id ===
                                      item.user_data[0]?.email
                                  )[0]
                                  ?.dispatch.map((newItem) => (
                                    <Row>
                                      <Col
                                        xs={6}
                                        xl={5}
                                      ...
                                      <Col xs={6} xl={5}>
                                        ...
                                        <span
                                          ...
                                          {newItem?.unitValueType}
                                        </span>{" "}
                                        :&nbsp;
                                        <label style={{}}>
                                          {newItem?.total_weight}&nbsp;
                                          {newItem?.unitValueType === "weight"
                                            ? "Kg"
                                            : newItem?.newItem
                                                ?.unitValueType === "liters"
                                            ? "Liters"
                                            : "Piece"}
                                        </label>
                                      </Col>
                ...
                <Tabs.TabPane tab="Ingredients for the menu" key="2">
                  <div
                    ...
                    <div
                      ...
                      <label>Daily Maintenance Charges :</label>
                      <Input
                        type="number"
                        name="dailyMaintenanceCharges"
                        onChange={onHandleChanges}
                        value={msciCharges?.dailyMaintenanceCharges}
                    ...
                    <div
                      ...
                      <label>Gas Charges :</label>
                      <Input
                        type="number"
                        name="gasCharges"
                        onChange={onHandleChanges}
                        value={msciCharges?.gasCharges}
                        placeholder="Eg: 2,3,15, etc"
                        style={{ width: "30%" }}
                      />
                    </div>
                    <div
                      ...
                      <Input
                        ...
                        name="laborCharges"
                        onChange={onHandleChanges}
                        value={msciCharges?.laborCharges}
                        placeholder="Eg: 2,3,15, etc"
                        style={{ width: "30%" }}
                      />
                    </div>
                    <div
                      ...
                      <Input
                        ...
                        onChange={onHandleChanges}
                        value={msciCharges?.transportCharges}
                        placeholder="Eg: 2,3,15, etc"
                        style={{ width: "30%" }}
                      />
                    </div>
                    <Button
                      onClick={() => {
                        setMsciChargesTotal(
                          Object.values(msciCharges).reduce(
                      ...
                      foodList.map((foodItem) => (
                        <div key={foodItem.food_item_id}>
                          <Row>
                            ...
                            {totalAshkash !== 0 && totalPrice.length !== 0 && (
                              <>
                                <Col xs={12} xl={8} style={{ padding: "1%" }}>
                                  <div style={{ fontSize: "1.3rem" }}>
                                ...
                                <Col
                                  xs={12}
                                  xl={8}
                                  style={{ padding: "1%", textAlign: "end" }}>
                                  <div style={{ fontSize: "1.3rem" }}>
                                    <span>
                                      Total cost for {foodItem.food_name}:{" "}
                                    </span>
                                    <span style={{ color: colorGreen }}>
                                      {totalPrice.length !== 0 &&
                                        (
                          ...
                          <List
                            dataSource={
                              pipelineData?.ingridient_list.length !== 0
                                ? pipelineData?.ingridient_list?.filter(
                                    (filterFoodItem) =>
                                      filterFoodItem.foodId ===
                                      foodItem.food_item_id
                                  )
                                : []
                            }
                                    ...
                                    <Col xs={12} xl={6}>
                                      ...
                                      item?.reorders.length !== 0 ? (
                                        <span
                                          ...
                                          +
                                          {item?.reorders.length !== 0 &&
                                            item?.reorders.reduce(
                                              (a, b) =>
                                                a + b.quantity_requireds,
                                              0
                                            )}
                                          &nbsp;
                                          {inventoryItems.length !== 0 &&
